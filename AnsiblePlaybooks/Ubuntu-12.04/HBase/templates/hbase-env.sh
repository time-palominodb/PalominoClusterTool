...
export HBASE_HEAPSIZE={{ hbase["hbase.heapsize"] }}

# for compression etc - 32- or 64-bit version
...
#export HBASE_OPTS="-ea -XX:+UseConcMarkSweepGC -XX:+CMSIncrementalMode -javaagent:/usr/java/jolokia-jvm-1.0.5-agent.jar=host=localhost,port=8086,agentContext=/jolokia"

# Uncomment below to enable java garbage collection logging in the .out file.
export HBASE_OPTS="$HBASE_OPTS {{ hbase["hbase.opts"] }}" 
...
export HBASE_IDENT_STRING={{ hbase["hbase.ident.string"] }}
