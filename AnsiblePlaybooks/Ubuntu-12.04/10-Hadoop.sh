...
# map/reduce daemons
JobTrackers=`awk '/\[.+\]/{m=0};NF && m{t++};/\[JobTrackers\]/{m=1} END{print t+0}' $ansibleHosts`
if [ $JobTrackers -lt 4 ] ; then echo "There must be 4 (or more) entries in [JobTrackers] section in $ansibleHosts - found $JobTrackers" ; exit 255 ; fi
